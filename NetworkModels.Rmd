...
: BIC of P1 models with binarized attributes
...
```{r include=FALSE}
bic_social <- sprintf("%.2f", BIC(social))
```
...
: Best models vs SRG (according to BIC)

## BIC summary table

| Model                     | BIC                               | Estimation |
|---------------------------|-----------------------------------|------------|
| srg_homo                  | `r bic_srg_homo`                  | MCMC       |
| main_Homo_Type_One        | `r bic_main_homo_type_one`        | MCMC       |
| main_Type_Two             | `r bic_main_type_two`             | MCMC       |
| main_Homo                 | `r bic_main_homo`                 | MCMC       |
| main_homo_type_one_binary | `r bic_main_homo_type_one_binary` | MCMC       |
| main_type_two_binary      | `r bic_main_type_two_binary`      | MCMC       |
| main_homo_binary          | `r bic_main_homo_binary`          | MCMC       |
| main_homo_type_one_binary | `r bic_main_homo_type_one_binary` | MCMC       |
| main_type_two_binary      | `r bic_main_type_two_binary`      | MCMC       |
| main_homo_binary          | `r bic_main_homo_binary`          | MCMC       |
| markov_no_triangles       | `r bic_markov_no_triangles`       | MCMC       |
| k_star                    | `r bic_k_star`                    | Stochastic |
| social                    | `r bic_social`                    | Stochastic |
...
dev.new()
png(file="Plots/Simulation/SRG.png")
par(mfrow = c(1,2))
hist(unlist(null.distr.srg[,2]),main="In-Degree SD", xlim = c(0.25,1.25),
     xlab = "in-degree");
abline(v = sd(degree(neurons_g, mode = "in")), col = "red")

hist(unlist(null.distr.srg[,3]), xlim = c(1,5.25),main="Out-Degree SD",
     xlab = "out-degree");
...
dev.off()
```
![SRG simulation](Plots/Simulation/SRG.png){width=50%}

## Markov without triangles simulation
```{r eval=FALSE, message=FALSE, warning=FALSE}
dev.new()
png(file="Plots/Simulation/Markov.png")
par(mfrow = c(1,2))
hist(unlist(null.distr.markov_no_triangles[,2]),main="In-Degree SD", xlab = "in-degree");
abline(v = sd(degree(neurons_g, mode = "in")), col = "red")

hist(unlist(null.distr.markov_no_triangles[,3]),main="Out-Degree SD",
     xlim = c(4,5.25),xlab = "out-degree");
abline(v = sd(degree(neurons_g, mode = "out")), col = "red")

mtext("Markov without triangles",
      side = 3,
      line = - 1,
      outer = TRUE)
dev.off()
```
![Markov simulation](Plots/Simulation/Markov.png){width=50%}

## Alternating k-stars simulation
```{r eval=FALSE, message=FALSE, warning=FALSE}
dev.new()
png(file="Plots/Simulation/Stars.png")
par(mfrow = c(1,2))
hist(unlist(null.distr.k_star[,2]),main="In-Degree SD",
     xlab = "in-degree");
abline(v = sd(degree(neurons_g, mode = "in")), col = "red")

hist(unlist(null.distr.k_star[,3]), main="Out-Degree SD",
     xlim = c(4,5.25),xlab = "out-degree");
abline(v = sd(degree(neurons_g, mode = "out")), col = "red")

mtext("Alternating k-stars",
      side = 3,
      line = - 1,
      outer = TRUE)
dev.off()
```
![Alternation k-star simulation](Plots/Simulation/Stars.png){width=50%}
